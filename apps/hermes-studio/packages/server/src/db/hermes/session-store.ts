...
function mapSessionRow(row: Record<string, unknown>): HermesSessionRow {
  ...
  return {
    ...
    memory_mode: row.memory_mode === 'clean' ? 'clean' : 'on',
...
export function createSession(data: {
  ...
  if (!isSqliteAvailable()) {
    return {
      ...
      memory_mode: data.memory_mode === 'clean' ? 'clean' : 'on',
  ...
  const db = getDb()!
  db.prepare(
    `INSERT INTO ${SESSIONS_TABLE} (id, profile, source, agent, agent_mode, agent_session_id, agent_native_session_id, memory_mode, model, provider, api_mode, title, parent_session_id, started_at, last_active, workspace)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(
    data.id,
...
export function createBranchedSession(data: {
  ...
  try {
    ...
    db.prepare(
      `INSERT INTO ${SESSIONS_TABLE} (id, profile, source, agent, agent_mode, agent_session_id, agent_native_session_id, memory_mode, model, provider, api_mode, title, parent_session_id, started_at, last_active, workspace, message_count)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run(
      data.id,
      data.profile || 'default',
