...
export async function bootstrap() {
  ...
  const configuredHost = String(config.host || '127.0.0.1').trim().toLowerCase()
  const loopbackOnly = configuredHost === '127.0.0.1' || configuredHost === 'localhost' || configuredHost === '::1' || configuredHost === '[::1]'
  const serverAddress = loopbackOnly
    ? `http://127.0.0.1:${config.port}`
    : `http://localhost:${config.port} (LAN: http://${localIp}:${config.port})`
  console.log(`Server: ${serverAddress}`)
  console.log(`Log: ${config.appHome}/logs/server.log`)
  logger.info('Server: %s', serverAddress)
  startLanDiscovery()
