...
export type AgentBridgeMemoryMode = 'on' | 'clean'
...
export interface AgentBridgeRequestOptions {
  timeoutMs?: number
  serialize?: boolean
  memory_mode?: AgentBridgeMemoryMode
}
...
export class AgentBridgeClient {
  ...
  private summarizePayload(payload: Record<string, unknown>): Record<string, unknown> {
    ...
    for (const key of ['session_id', 'run_id', 'request_id', 'approval_id', 'profile', 'worker_key', 'memory_mode']) {
  ...
  chat(
    ...
    return this.request<AgentBridgeChatStarted>({
      ...
      ...(options.memory_mode ? { memory_mode: options.memory_mode } : {}),
    })
  }
  ...
  contextEstimate(
    ...
    instructions?: string,
    profile?: string,
    options: Pick<AgentBridgeChatOptions, 'model' | 'provider' | 'workspace' | 'memory_mode'> = {},
  ...
  status(sessionId: string, profile?: string, memoryMode?: AgentBridgeMemoryMode): Promise<AgentBridgeResponse> {
    return this.request({
      action: 'status',
      session_id: sessionId,
      ...(profile ? { profile } : {}),
  ...
  statusIfLoaded(sessionId: string, profile?: string, options: AgentBridgeRequestOptions = {}, memoryMode?: AgentBridgeMemoryMode): Promise<AgentBridgeResponse> {
    return this.request({
      action: 'status_if_loaded',
      session_id: sessionId,
      ...(profile ? { profile } : {}),
      ...((memoryMode || options.memory_mode) ? { memory_mode: memoryMode || options.memory_mode } : {}),
    }, options)
  }
