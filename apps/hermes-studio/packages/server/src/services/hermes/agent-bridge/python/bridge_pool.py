...
class AgentPool:
    ...
    def get_or_create(
        ...
        requested_memory_mode = "clean" if str(memory_mode or "").strip().lower() == "clean" else "on"
        with self._lock:
            ...
            if existing is not None:
                ...
                memory_mode_changed = existing.config.get("memory_mode", "on") != requested_memory_mode
                config_changed = profile_changed or runtime_changed or memory_mode_changed
                if config_changed:
                    if (profile_changed or memory_mode_changed) and not existing.running:
                        self._destroy_session(session_id)
                    elif profile_changed or memory_mode_changed:
            ...
            with _profile_env(profile):
                ...
                if requested_memory_mode == "clean" and isinstance(enabled_toolsets, list):
                    enabled_toolsets = [
                        item for item in enabled_toolsets
                        if "memory" not in str(item).lower() and "session_search" not in str(item).lower()
                    ]
                ...
                agent = AIAgent(
                    ...
                    skip_memory=requested_memory_mode == "clean",
                )
                if requested_memory_mode == "clean":
                    self._isolate_clean_memory_agent(agent)
    ...
    @staticmethod
    def _clean_tool_name(tool: Any) -> str:
        if isinstance(tool, dict):
            function = tool.get("function") if isinstance(tool.get("function"), dict) else {}
            return str(tool.get("name") or function.get("name") or "").lower()
        return str(getattr(tool, "name", "") or getattr(tool, "__name__", "")).lower()

    def _isolate_clean_memory_agent(self, agent: Any) -> None:
        """Remove memory surfaces not covered by older skip_memory runtimes."""
        tools = getattr(agent, "tools", None)
        if isinstance(tools, list):
            agent.tools = [
                tool for tool in tools
                if "memory" not in self._clean_tool_name(tool)
                and "session_search" not in self._clean_tool_name(tool)
            ]
        elif isinstance(tools, dict):
            agent.tools = {
                key: value for key, value in tools.items()
                if "memory" not in str(key).lower() and "session_search" not in str(key).lower()
            }

        for attr in ("memory_provider", "memory_service", "memory_manager", "external_memory_provider"):
            if hasattr(agent, attr):
                try:
                    setattr(agent, attr, None)
                except Exception:
                    pass
        for attr in ("_cached_system_prompt", "_system_prompt_cache", "system_prompt_cache"):
            if hasattr(agent, attr):
                try:
                    setattr(agent, attr, None)
                except Exception:
    ...
    def _bridge_context_ready_event(self, session: AgentSession, instructions: str | None, profile: str | None) -> dict[str, Any]:
        ...
        event = {
            ...
            "memory_mode": session.config.get("memory_mode", "on"),
            **info,
        }
    ...
    def status(self, session_id: str, memory_mode: str | None = None) -> dict[str, Any]:
        with self._lock:
            session = self._sessions.get(session_id)
        if session is None:
            return {
                ...
                "running": False,
                "message_count": 0,
                "memory_mode": "clean" if str(memory_mode or "").lower() == "clean" else "on",
            }
        with session.lock:
            return {
                "session_id": session_id,
