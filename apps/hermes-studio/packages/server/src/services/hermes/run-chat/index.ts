...
import type { ChatCodingAgentId, ContentBlock, QueuedRun, SessionMemoryMode, SessionState } from './types'
...
export class ChatRunSocket {
  ...
  private onConnection(socket: Socket) {
    ...
    socket.on('run', async (data: {
      ...
      if (data.session_id) {
        ...
        state.memoryMode = data.memory_mode === 'clean' ? 'clean' : (getSession(data.session_id)?.memory_mode || 'on')
  ...
  private async resumeSession(socket: Socket, sid: string) {
    ...
    socket.emit('resumed', {
      ...
      memory_mode: state.memoryMode || sessionDetail?.memory_mode || 'on',
  ...
  private async reattachBridgeRun(socket: Socket, sid: string, state: SessionState) {
    ...
    try {
      const status = await this.bridge.statusIfLoaded(sid, profile, { timeoutMs: 1000 }, session?.memory_mode) as Record<string, unknown>
  ...
  async runAndWait(
    ...
    state.profile = profile
    state.source = source
    state.memoryMode = data.memory_mode === 'clean' ? 'clean' : (getSession(sessionId)?.memory_mode || 'on')
