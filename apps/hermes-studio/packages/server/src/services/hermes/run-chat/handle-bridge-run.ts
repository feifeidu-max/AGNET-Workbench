...
import type { ContentBlock, QueuedRun, SessionMemoryMode, SessionState } from './types'
...
function cacheBridgeContext(state: SessionState, data: Record<string, unknown> | AgentBridgeContextEstimate, workspace?: string | null) {
  ...
  state.bridgeContext = {
    ...
    memoryMode: data.memory_mode === 'clean' ? 'clean' : data.memory_mode === 'on' ? 'on' : state.memoryMode,
...
function bridgeContextMatches(
  state: SessionState,
  expected: { profile: string; model?: string | null; provider?: string | null; workspace?: string | null; memoryMode?: SessionMemoryMode },
): boolean {
  ...
  if (expected.memoryMode && context.memoryMode && context.memoryMode !== expected.memoryMode) return false
  return true
}
...
export async function handleBridgeRun(
  ...
  const requestedMemoryMode: SessionMemoryMode = data.memory_mode === undefined
    ? (sessionRow?.memory_mode || 'on')
    : data.memory_mode === 'clean' ? 'clean' : 'on'
  if (sessionRow && sessionRow.message_count > 0 && data.memory_mode !== undefined && sessionRow.memory_mode !== requestedMemoryMode) {
    socket.emit('run.failed', {
      event: 'run.failed',
      session_id,
      error: 'Memory mode is locked after the first message; create a new session to change it',
      code: 'MEMORY_MODE_LOCKED',
      memory_mode: sessionRow.memory_mode,
    })
    return
  }
  let memoryMode: SessionMemoryMode = sessionRow?.memory_mode || requestedMemoryMode
  if (sessionRow && sessionRow.message_count === 0 && data.memory_mode !== undefined && sessionRow.memory_mode !== requestedMemoryMode) {
    updateSession(session_id, { memory_mode: requestedMemoryMode })
    // The mode change applies to this first request as well as future turns.
    // Do not let the stale session row leak MEMORY/USER into a clean run.
  ...
  if (shouldPersistUserMessage) {
    ...
    if (!getSession(session_id)) {
      ...
      createSession({ id: session_id, profile, source: runSource, memory_mode: memoryMode, model: resolvedModel, provider: resolvedProvider, title: preview, workspace })
    }
...
export async function resumeBridgeRun(
  ...
  state.memoryMode = getSession(sessionId)?.memory_mode || state.memoryMode || 'on'
  state.runId = runId
  state.activeRunMarker = runMarker
...
async function refreshFinalContextUsage(args: {
  ...
  try {
    ...
    await ensureBridgeFixedContext({
      ...
      memoryMode: args.state.memoryMode || 'on',
