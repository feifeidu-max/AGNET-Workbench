  ...
  if (command.name === 'skill') {
    ...
    if (result.handled && expandedPrompt && (result.type === 'skill' || result.type === 'bundle')) {
      ...
      const next: QueuedRun = {
        ...
        profile: ctx.profile,
        source: 'cli',
        memoryMode: getSession(sessionId)?.memory_mode || state.memoryMode || 'on',
...
async function getBridgeSessionStatus(ctx: SessionCommandContext, sessionId: string): Promise<BridgeSessionStatus | null> {
  try {
    const raw = await ctx.bridge.status(sessionId, ctx.profile, getSession(sessionId)?.memory_mode) as Record<string, unknown>
    return {
...
function ensureCommandSession(sessionId: string, command: ParsedSessionCommand, ctx: SessionCommandContext) {
  ...
  createSession({
    ...
    source: 'cli',
    model: ctx.model,
    memory_mode: ctx.memoryMode === 'clean' ? 'clean' : 'on',
