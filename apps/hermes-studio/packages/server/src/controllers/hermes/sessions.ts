...
export async function setMemoryMode(ctx: any) {
  const requested = (ctx.request.body as { memory_mode?: unknown; memoryMode?: unknown })?.memory_mode
    ?? (ctx.request.body as { memoryMode?: unknown })?.memoryMode
  if (requested !== 'on' && requested !== 'clean') {
    ctx.status = 400
    ctx.body = { error: 'memory_mode must be "on" or "clean"' }
    return
  }
  ...
  const existing = localGetSession(id)
  if (denySessionAccess(ctx, existing)) return
  if (existing && existing.message_count > 0 && existing.memory_mode !== requested) {
    ctx.status = 409
    ctx.body = {
  ...
  if (!existing) {
    localCreateSession({
      ...
      title: '',
      memory_mode: requested,
    })
  } else if (existing.memory_mode !== requested) {
    localUpdateSession(id, { memory_mode: requested })
  }
  ctx.body = { ok: true, memory_mode: requested, locked: Boolean(existing?.message_count) }
}
