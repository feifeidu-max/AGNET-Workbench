...
export interface MemoryData {
  ...
  sections?: Record<'memory' | 'user' | 'soul', { path: string; revision: number; mtime: number | null }>
  character_budget?: { max_chars: number; memory: number; user: number; soul: number }
  effective_status?: { memory_enabled: boolean; user_profile_enabled: boolean; soul_always_loaded: boolean; clean_mode_excludes_profile_files: boolean }
}
...
export async function saveMemory(section: 'memory' | 'user' | 'soul', content: string, expectedRevision?: number): Promise<{ revision?: number; mtime?: number; path?: string }> {
  return request('/api/hermes/memory', {
    method: 'POST',
    headers: expectedRevision != null ? { 'If-Match': `"${expectedRevision}"` } : undefined,
    body: JSON.stringify({ section, content, ...(expectedRevision != null ? { expectedRevision } : {}) }),
  })
}
...
export async function fetchMemoryHistory(section: 'memory' | 'user' | 'soul'): Promise<{ section: string; path: string; current_revision: number; history: MemoryHistoryEntry[] }> {
  return request(`/api/hermes/memory/history?section=${encodeURIComponent(section)}`)
}

export async function restoreMemory(section: 'memory' | 'user' | 'soul', revision: number, expectedRevision?: number): Promise<void> {
  await request('/api/hermes/memory/restore', {
    method: 'POST',
    headers: expectedRevision != null ? { 'If-Match': `"${expectedRevision}"` } : undefined,
    body: JSON.stringify({ section, revision, ...(expectedRevision != null ? { expectedRevision } : {}) }),
  })
}
