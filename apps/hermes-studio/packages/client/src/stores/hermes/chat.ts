...
import { archiveSession as archiveSessionApi, deleteSession as deleteSessionApi, fetchSessionMessagesPage, fetchSessions, fetchWorkspaceRunChangeFile, fetchWorkspaceRunChangesForSession, setSessionMemoryMode as setSessionMemoryModeApi, setSessionModel, type HermesMessage, type SessionMemoryMode, type SessionSummary, type WorkspaceRunChangeFileDetail, type WorkspaceRunChangeSummary } from '@/api/hermes/sessions'
...
function mapHermesSession(s: SessionSummary): Session {
  ...
  return {
    ...
    memoryMode: s.memory_mode === 'clean' ? 'clean' : 'on',
  }
}
...
export const useChatStore = defineStore('chat', () => {
  ...
  // The composer can be shown before the first local session exists. Keep the
  // user's memory choice here until sendMessage materializes that session.
  const pendingMemoryMode = ref<SessionMemoryMode>('on')
  ...
  async function refreshActiveSession(): Promise<boolean> {
    ...
    try {
      ...
      target.memoryMode = detail.session.memory_mode === 'clean' ? 'clean' : 'on'
  ...
  function createSession(options: {
    ...
    const session: Session = {
      ...
      memoryMode: options.memoryMode === 'clean' ? 'clean' : 'on',
    }
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        resumeSession(sessionId, (data) => {
          ...
          target.memoryMode = data.memory_mode === 'clean' ? 'clean' : target.memoryMode || 'on'
      ...
      if (newSessionId) {
        ...
        if (!existing) {
          sessions.value.unshift({
            ...
            memoryMode: branch.memoryMode === 'clean' ? 'clean' : target?.memoryMode || 'on',
          })
        }
  ...
  async function sendMessage(content: string, attachments?: Attachment[]) {
    ...
    if (!activeSession.value) {
      const session = createSession({ memoryMode: pendingMemoryMode.value })
      // The pending choice is only for this not-yet-created session. A later
      // blank composer starts from the standard memory-on default.
      pendingMemoryMode.value = 'on'
      switchSession(session.id)
    }
    ...
    try {
      ...
      const runPayload: StartRunRequest = {
        ...
        memory_mode: activeSession.value?.memoryMode || 'on',
      }
      ...
      if (document.visibilityState === 'visible' && activeSessionId.value && !isStreaming.value) {
        ...
        if (sid && !streamStates.value.has(sid)) {
          ...
          resumeSession(sid, (data) => {
            ...
            if (!data.isWorking) setCompressionState(sid, null)
            if (activeSession.value) {
              activeSession.value.memoryMode = data.memory_mode === 'clean' ? 'clean' : activeSession.value.memoryMode || 'on'
            }
  ...
  async function setSessionMemoryMode(sessionId: string, memoryMode: SessionMemoryMode): Promise<boolean> {
    const target = sessions.value.find(session => session.id === sessionId)
    if (!target) return false
    const hasMessages = (target.messageCount || 0) > 0
      || target.messages.some(item => item.role === 'user' || item.role === 'assistant' || item.role === 'command')
    if (hasMessages && target.memoryMode !== memoryMode) return false
    if (!target.isLocalOnly && target.memoryMode !== memoryMode) {
      const ok = await setSessionMemoryModeApi(sessionId, memoryMode)
      if (!ok) return false
    }
    target.memoryMode = memoryMode
    if (activeSession.value?.id === sessionId) activeSession.value.memoryMode = memoryMode
    return true
  }

  function setPendingMemoryMode(memoryMode: SessionMemoryMode) {
    pendingMemoryMode.value = memoryMode === 'clean' ? 'clean' : 'on'
  }
