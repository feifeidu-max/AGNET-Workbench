...
const renderedHtml = computed(() => {
  ...
  html = html.replace(/<a href="([^"]+)">([^<]+)<\/a>/g, (match, rawPath, filename) => {
    // These are authenticated Studio routes, not filesystem paths. Keep the
    // anchor so the click handler below can attach the current browser JWT.
    if (
      !isLocalFilePath(rawPath)
      || rawPath.startsWith('/api/hermes/download?')
      || rawPath.startsWith('/api/knowledge/sources/')
    ) return match
...
async function handleMarkdownClick(event: MouseEvent): Promise<void> {
  ...
  // Knowledge citations point through the Studio BFF. Add the browser JWT
  // because a normal anchor cannot send the Authorization header.
  if (href.startsWith('/api/knowledge/sources/')) {
    event.preventDefault()
    const token = getApiKey()
    const target = new URL(href, window.location.origin)
    if (token) target.searchParams.set('token', token)
    const page = Math.max(1, Number(target.searchParams.get('page')) || 1)
