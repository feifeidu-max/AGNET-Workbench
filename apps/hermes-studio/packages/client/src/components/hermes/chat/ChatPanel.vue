...
const newChatMemoryMode = ref<"on" | "clean">("on");
...
async function confirmNewChat() {
  ...
  const session = chatStore.newChat({
    ...
    memoryMode: newChatAgent.value === "hermes" ? newChatMemoryMode.value : "on",
  });
...
const activeMemoryModeLocked = computed(() => {
  const session = chatStore.activeSession;
  if (!session) return false;
  return (session.messageCount || 0) > 0
    || session.messages.some(item => item.role === "user" || item.role === "assistant" || item.role === "command");
});

async function handleHeaderMemoryModeClick() {
  const session = chatStore.activeSession;
  if (!session) return;
  if (activeMemoryModeLocked.value) {
    message.info(t("chat.memoryMode.lockedHint"));
    return;
  }
  const next = session.memoryMode === "clean" ? "on" : "clean";
  const updated = await chatStore.setSessionMemoryMode(session.id, next);
  if (!updated) message.warning(t("chat.memoryMode.updateFailed"));
}
    ...
    >
      <NDrawerContent :title="t('chat.newChat')" closable>
        <div class="new-chat-form">
          ...
          <label v-if="newChatAgent === 'hermes'" class="new-chat-field">
            <span class="new-chat-label">{{ t("chat.memoryMode.title") }}</span>
            <NRadioGroup v-model:value="newChatMemoryMode" name="new-chat-memory-mode">
              <NRadioButton value="on">{{ t("chat.memoryMode.on") }}</NRadioButton>
              <NRadioButton value="clean">{{ t("chat.memoryMode.clean") }}</NRadioButton>
            </NRadioGroup>
            <span class="new-chat-memory-hint">
              {{ newChatMemoryMode === 'on' ? t("chat.memoryMode.onHint") : t("chat.memoryMode.cleanHint") }}
            </span>
          </label>
          <label class="new-chat-field">
    ...
    <div class="chat-main">
      <header class="chat-header">
        <div class="header-left">
          ...
          <button
            v-if="currentMode === 'chat' && chatStore.activeSession && chatStore.activeSession.source !== 'coding_agent'"
            class="memory-mode-badge"
            :class="{ clean: chatStore.activeSession.memoryMode === 'clean', locked: activeMemoryModeLocked }"
            type="button"
            :title="activeMemoryModeLocked ? t('chat.memoryMode.lockedHint') : t('chat.memoryMode.switchHint')"
            @click="handleHeaderMemoryModeClick"
          >
            <span class="memory-mode-dot" aria-hidden="true" />
            {{ chatStore.activeSession.memoryMode === 'clean' ? t("chat.memoryMode.clean") : t("chat.memoryMode.on") }}
          </button>
          <button
...
.memory-mode-badge {
  ...
  &.clean {
    border-color: rgba(var(--accent-info-rgb), 0.38);
