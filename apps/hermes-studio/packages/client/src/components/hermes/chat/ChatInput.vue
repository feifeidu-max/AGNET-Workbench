...
const activeMemoryMode = computed(() =>
  chatStore.activeSession?.memoryMode || chatStore.pendingMemoryMode || 'on',
)
const showsMemoryMode = computed(() => chatStore.activeSession?.source !== 'coding_agent')
const isMemoryModeLocked = computed(() => {
...
async function handleMemoryModeClick() {
  const session = chatStore.activeSession
  if (!session) {
    const next = activeMemoryMode.value === 'clean' ? 'on' : 'clean'
    chatStore.setPendingMemoryMode(next)
    return
  }
  if (isMemoryModeLocked.value) {
    message.info(t('chat.memoryMode.lockedHint'))
    return
  }
  const next = activeMemoryMode.value === 'clean' ? 'on' : 'clean'
  if (!await chatStore.setSessionMemoryMode(session.id, next)) {
    message.warning(t('chat.memoryMode.updateFailed'))
  }
}
    ...
    >
      ...
      <div class="input-toolbar">
        ...
        <div class="input-top-bar">
          ...
          <NTooltip v-if="showsMemoryMode" trigger="hover" :disabled="isMobileViewport">
            <template #trigger>
              <NButton
                quaternary
                size="tiny"
                class="memory-mode-button"
                :class="{ clean: activeMemoryMode === 'clean', locked: isMemoryModeLocked }"
                :aria-label="t('chat.memoryMode.title')"
                @click="handleMemoryModeClick"
              >
                <template #icon>
                  <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M12 5a3 3 0 0 0-5.8 1.1A3.5 3.5 0 0 0 5 12.8 3 3 0 0 0 8 18h4" />
                    <path d="M12 5a3 3 0 0 1 5.8 1.1 3.5 3.5 0 0 1 1.2 6.7A3 3 0 0 1 16 18h-4V5Z" />
                  </svg>
                </template>
                <span class="memory-mode-label">{{ activeMemoryMode === 'clean' ? t('chat.memoryMode.clean') : t('chat.memoryMode.on') }}</span>
              </NButton>
            </template>
            {{ isMemoryModeLocked ? t('chat.memoryMode.lockedHint') : t('chat.memoryMode.switchHint') }}
          </NTooltip>
