...
const requiresCredentialChange = ref(false);
...
onMounted(async () => {
  try {
    ...
    requiresCredentialChange.value = user.requiresCredentialChange === true;
  } catch { /* ignore */ }
  try {
...
async function handleChangePassword() {
  ...
  loading.value = true;
  try {
    const shouldReconnect = requiresCredentialChange.value;
    await changePassword(currentPasswordForPwd.value, newPasswordVal.value);
    requiresCredentialChange.value = false;
