export default {
  ...
  login: {
    ...
    defaultCredentialTitle: '请修改默认密码',
    defaultCredentialMessage: '当前登录账户仍在使用默认密码。为避免未授权访问，必须先修改密码才能继续使用工作台。',
  ...
  chat: {
    ...
    memoryMode: {
      ...
      on: '开启记忆',
      clean: '干净上下文',
      onHint: 'Hermes 可读取长期 MEMORY.md 和 USER.md。',
      cleanHint: '排除 MEMORY.md 和 USER.md，但保留 SOUL.md 与工作区上下文。',
      switchHint: '首条消息前可点击切换。',
      lockedHint: '首条消息后记忆模式已锁定，请新建会话后更改。',
      updateFailed: '记忆模式更新失败。',
    },
  ...
  memory: {
    ...
    publicModelWarning: '开启记忆会将 MEMORY.md 和 USER.md 发送给配置的公网模型，请勿写入敏感信息。',
    revisionConflict: '记忆已在其他位置修改，已加载最新版本，请确认后重新保存。',
    history: '版本历史',
    historyLoadFailed: '加载版本历史失败',
    noHistory: '暂无已保存版本。',
    currentRevision: '当前版本',
    restore: '恢复',
    restoreConfirm: '确认将此版本恢复为一个新版本吗？',
    restoreSuccess: '版本已恢复',
    restoreFailed: '恢复版本失败',
    characterBudget: '字符预算',
  ...
  settings: {
    ...
    models: {
      apiKey: 'API Key',
      sensenova: {
        title: 'SenseNova 模型接入',
        description: '为 Hermes 配置 OpenAI 兼容的 SenseNova 端点。密钥仅保存在本机，读取时只显示掩码。',
        configured: '已配置',
        notConfigured: '未配置',
        endpoint: '接入地址',
        endpointPlaceholder: 'https://token.sensenova.cn/v1',
        apiKey: 'API Key',
        keyPlaceholder: '填写 SenseNova API Key',
        keyConfigured: '已配置（{hint}），留空表示保持不变',
        keyHint: '填写新密钥会替换当前密钥；留空则继续使用现有密钥。',
        model: '默认模型',
        modelPlaceholder: '先测试获取模型，或直接填写模型 ID',
        test: '测试连接',
        save: '保存接入',
        testing: '正在测试连接…',
        saving: '正在保存…',
        testSuccess: '连接正常，获取到 {count} 个模型',
        testReady: 'SenseNova 可访问，模型列表已加载。',
        testFailed: 'SenseNova 连接失败，请检查地址、密钥和网络。',
        loadFailed: '读取 SenseNova 设置失败',
        saveSuccess: 'SenseNova 接入已保存',
        saveFailed: '保存 SenseNova 设置失败',
        required: '接入地址和模型不能为空',
        modelUnavailable: '接口未返回当前模型；如果平台支持该模型，仍可继续保存。',
      },
