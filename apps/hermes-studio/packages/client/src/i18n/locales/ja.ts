export default {
  ...
  login: {
    ...
    defaultCredentialTitle: '既定のパスワードを変更してください',
    defaultCredentialMessage: 'このアカウントは既定のパスワードを使用しています。ワークベンチを続ける前にパスワードを変更してください。',
  ...
  chat: {
    ...
    memoryMode: {
      ...
      on: 'メモリー有効',
      clean: 'クリーンコンテキスト',
      onHint: 'Hermes は MEMORY.md と USER.md を利用します。',
      cleanHint: 'MEMORY.md と USER.md を除外し、SOUL.md とワークスペースを保持します。',
      switchHint: '最初のメッセージ前に切り替えできます。',
      lockedHint: '最初のメッセージ後は固定されます。変更するには新しいセッションを作成してください。',
      updateFailed: 'メモリーモードを更新できませんでした。',
    },
  ...
  memory: {
    ...
    publicModelWarning: 'メモリーを有効にすると MEMORY.md と USER.md が公開モデルへ送信されます。機密情報を保存しないでください。',
    revisionConflict: 'メモリーが別の場所で変更されました。最新バージョンを読み込みました。確認して再保存してください。',
    characterBudget: '文字数上限',
    effectiveStatus: '有効状態',
    enabled: '有効',
    disabled: '無効',
  },
