...
describe('ChatInput draft persistence', () => {
  ...
  it('keeps the memory choice in the composer before the first session exists', async () => {
    const { wrapper, chatStore } = mountWithoutSession()
    const button = wrapper.get('.memory-mode-button')

    expect(button.text()).toContain('chat.memoryMode.on')

    await button.trigger('click')
    await nextTick()

    expect(chatStore.pendingMemoryMode).toBe('clean')
    expect(button.text()).toContain('chat.memoryMode.clean')

    await button.trigger('click')
    expect(chatStore.pendingMemoryMode).toBe('on')
  })
