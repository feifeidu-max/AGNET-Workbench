...
describe('MarkdownRenderer', () => {
  ...
  it('keeps knowledge citations as links and adds the browser JWT when opened', async () => {
    localStorage.setItem('hermes_api_key', 'test-browser-jwt')
    const open = vi.spyOn(window, 'open').mockImplementation(() => null)
    const wrapper = mount(MarkdownRenderer, {
      props: {
        content: '[【Ada Lovelace et al., 2025, p.4】](/api/knowledge/sources/paper%3Aabc/pdf?page=4)',
      },
    })

    expect(wrapper.find('.markdown-file-card').exists()).toBe(false)
    const citation = wrapper.find('a')
    expect(citation.attributes('href')).toBe('/api/knowledge/sources/paper%3Aabc/pdf?page=4')

    await citation.trigger('click')

    expect(open).toHaveBeenCalledWith(
      '/api/knowledge/sources/paper%3Aabc/pdf?page=4&token=test-browser-jwt#page=4',
      '_blank',
      'noopener,noreferrer',
    )
