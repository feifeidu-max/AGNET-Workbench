...
const setMemoryModeMock = vi.fn(async (ctx: any) => { ctx.body = { ok: true, memory_mode: 'clean' } })
...
vi.mock('../../packages/server/src/controllers/hermes/sessions', () => ({
  ...
  setMemoryMode: setMemoryModeMock,
...
describe('session routes', () => {
  ...
  it('registers conversations, session list, and search routes', async () => {
    ...
    expect(paths).toEqual(expect.arrayContaining([
      ...
      '/api/hermes/sessions/:id/memory-mode',
  ...
  it('delegates session memory mode changes to the controller', async () => {
    const { sessionRoutes } = await import('../../packages/server/src/routes/hermes/sessions')
    const layer = sessionRoutes.stack.find((entry: any) => entry.path === '/api/hermes/sessions/:id/memory-mode')
    const ctx: any = { request: { body: { memory_mode: 'clean' } }, body: null, params: { id: 'session-1' } }

    await layer.stack[0](ctx)

    expect(setMemoryModeMock).toHaveBeenCalledWith(ctx)
    expect(ctx.body).toEqual({ ok: true, memory_mode: 'clean' })
  })
