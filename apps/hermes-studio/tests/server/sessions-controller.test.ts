...
describe('session conversations controller', () => {
  ...
  it('creates an empty session with its selected clean memory mode', async () => {
    getSessionMock.mockReturnValue(null)
    const mod = await import('../../packages/server/src/controllers/hermes/sessions')
    const ctx: any = {
      params: { id: 'clean-session' },
      request: { body: { memory_mode: 'clean' } },
      body: null,
    }

    await mod.setMemoryMode(ctx)

    expect(localCreateSessionMock).toHaveBeenCalledWith(expect.objectContaining({
      id: 'clean-session',
      memory_mode: 'clean',
    }))
    expect(ctx.body).toEqual({ ok: true, memory_mode: 'clean', locked: false })
  })

  it('locks memory mode after the first session message', async () => {
    getSessionMock.mockReturnValue({ id: 'session-1', memory_mode: 'on', message_count: 1 })
    const mod = await import('../../packages/server/src/controllers/hermes/sessions')
    const ctx: any = {
      params: { id: 'session-1' },
      request: { body: { memory_mode: 'clean' } },
    ...
    expect(ctx.body).toEqual(expect.objectContaining({ code: 'MEMORY_MODE_LOCKED', memory_mode: 'on' }))
    expect(localUpdateSessionMock).not.toHaveBeenCalled()
  })

  it('updates memory mode while a session is still empty', async () => {
    getSessionMock.mockReturnValue({ id: 'session-1', memory_mode: 'on', message_count: 0 })
    const mod = await import('../../packages/server/src/controllers/hermes/sessions')
    const ctx: any = {
      params: { id: 'session-1' },
      request: { body: { memoryMode: 'clean' } },
      body: null,
    }

    await mod.setMemoryMode(ctx)

    expect(localUpdateSessionMock).toHaveBeenCalledWith('session-1', { memory_mode: 'clean' })
    expect(ctx.body).toEqual({ ok: true, memory_mode: 'clean', locked: false })
  })

  it('rejects invalid session memory modes', async () => {
    const mod = await import('../../packages/server/src/controllers/hermes/sessions')
    const ctx: any = {
      params: { id: 'session-1' },
      request: { body: { memory_mode: 'off' } },
      body: null,
      status: 200,
    }
