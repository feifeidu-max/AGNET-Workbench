...
describe('bridge run final context usage', () => {
  ...
  it('applies an empty session memory-mode change to the first bridge request', async () => {
    getSessionMock.mockReturnValue({
      id: 'session-1',
      profile: 'default',
      model: 'gpt-test',
    ...
    const bridge = {
      chat: vi.fn().mockResolvedValue({ run_id: 'run-clean', status: 'started' }),
      contextEstimate: vi.fn().mockResolvedValue({ token_count: 10, fixed_context_tokens: 5 }),
      streamOutput: vi.fn(async function* () {
        yield { run_id: 'run-clean', done: true, status: 'completed', output: 'done' }
      }),
    } as any
    ...
    await handleBridgeRun(
      makeNamespace(vi.fn()),
      makeSocket(),
      { input: 'clean request', session_id: 'session-1', memory_mode: 'clean' },
      'default',
    ...
    expect(updateSessionMock).toHaveBeenCalledWith('session-1', { memory_mode: 'clean' })
    expect(bridge.chat.mock.calls[0][5]).toEqual(expect.objectContaining({ memory_mode: 'clean' }))
  })
