...
describe('user auth tables and middleware', () => {
  ...
  it('falls back to the default user JWT lifetime for invalid overrides', async () => {
    ...
    expect(auth.getUserJwtExpiresSeconds()).toBe(12 * 60 * 60)
  })
  ...
  it('blocks workbench APIs until the bootstrap password is changed', async () => {
    ...
    await auth.requireCredentialChange(ctx, next)

    expect(next).not.toHaveBeenCalled()
    expect(ctx.status).toBe(428)
    expect(ctx.body).toEqual(expect.objectContaining({ code: 'CREDENTIAL_CHANGE_REQUIRED' }))
  })

  it('blocks socket and websocket token authentication until the bootstrap password is changed', async () => {
    const { users, auth } = await initUsers()
    const user = users.bootstrapDefaultSuperAdmin('admin', '123456')!
    const token = await auth.issueUserJwt(user)

    expect(await auth.authenticateUserToken(token)).toBeNull()

    users.updateUserPassword(user.id, 'new-local-password')
    expect(await auth.authenticateUserToken(token)).toEqual(expect.objectContaining({
      id: user.id,
      username: 'admin',
      role: 'super_admin',
    }))
  ...
  it('allows credential routes and unlocks APIs after the password changes', async () => {
    const { users, auth } = await initUsers()
    const user = users.bootstrapDefaultSuperAdmin('admin', '123456')!
    const credentialCtx = {
    ...
    const credentialNext = vi.fn(async () => {})
    await auth.requireCredentialChange(credentialCtx, credentialNext)
    expect(credentialNext).toHaveBeenCalledOnce()
    ...
    const workbenchNext = vi.fn(async () => {})
    await auth.requireCredentialChange(workbenchCtx, workbenchNext)
    expect(workbenchNext).toHaveBeenCalledOnce()
  })
  ...
  it('requires a credential change for any account still using password 123456', async () => {
    ...
    expect(usernameChangedCtx.body.user.requiresCredentialChange).toBe(true)
  })
