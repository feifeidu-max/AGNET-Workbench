...
export async function mockHermesApi(page: Page, options: MockHermesApiOptions = {}) {
  ...
  await page.route('**/*', async (route: Route) => {
    ...
    if (pathname === '/api/workbench/summary') {
      await route.fulfill(jsonResponse({
        knowledge: { drafts: 0, trusted: 0, candidates: 0, serviceOk: true, todayPapers: 0 },
        company: { metricCount: 0, lastUpdated: null, status: 'not_run' },
        reports: { nextRun: null, lastStatus: 'not_run' },
        services: [],
      }))
      return
    }

    if (pathname === '/api/hermes/memory' && request.method() === 'GET') {
      await route.fulfill(jsonResponse(options.memory ?? {
        memory: '',
        user: '',
        soul: '',
        memory_mtime: null,
