...
function researchToolsetEnabled(): boolean {
  return process.env.LLM_WIKI_MCP_TOOLSET?.trim().toLowerCase() === "research"
}
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  ...
  return {
    tools: researchToolsetEnabled()
      ? tools.filter((tool) => RESEARCH_TOOLS.has(tool.name))
      : tools,
  }
})
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    if (researchToolsetEnabled() && !RESEARCH_TOOLS.has(request.params.name)) {
      throw new McpError(
        ErrorCode.MethodNotFound,
        `Tool '${request.params.name}' is disabled by the research profile`,
      )
    }
    switch (request.params.name) {
      ...
      case "llm_wiki_search": {
        ...
        if (localEvidenceIsSufficient(search, numberArg(args.top_k))) {
          return textResult(formatLocalFirstSearch(query, search))
        }
        try {
          const external = await client.searchReadingCandidates(projectId(args), query)
          return textResult(formatLocalFirstSearch(query, search, external))
        } catch (error) {
          const message = error instanceof Error ? error.message : String(error)
          return textResult(formatLocalFirstSearch(query, search, undefined, message))
        }
      }
      case "llm_wiki_chat": {
        await assertMcpEnabled()
