...
test("reading candidate search posts providers and parses external evidence", async () => {
  let url = ""
  let body = ""
  const fetchImpl = async (requestUrl: string | URL | Request, init?: RequestInit): Promise<Response> => {
    ...
    return new Response(JSON.stringify({
      ...
      candidates: [{
        ...
        query: "query",
        recommendedReason: "Related external paper from Crossref",
        status: "unread",
      }],
      providerErrors: ["arxiv: timeout"],
    }), { status: 200 })
  }
  const client = new LlmWikiApiClient({ baseUrl: "http://localhost:19828", fetchImpl })
  const response = await client.searchReadingCandidates("current", "query", {
    providers: ["crossref", "arxiv"],
  })

  assert.equal(url, "http://localhost:19828/api/v1/projects/current/reading-candidates/search")
  assert.deepEqual(JSON.parse(body), { query: "query", providers: ["crossref", "arxiv"] })
  assert.equal(response.candidates[0]?.authors[0], "Ada Lovelace")
  assert.equal(response.providerErrors[0], "arxiv: timeout")
})
