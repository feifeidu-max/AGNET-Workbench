...
import { startStrictIngestWorker, stopStrictIngestWorker } from "@/lib/strict-ingest-worker"
...
function App() {
  ...
  const [strictWorkerProject, setStrictWorkerProject] = useState<WikiProject | null>(null)

  // Strict ingestion is a project invariant: every staged source must pass
  // the isolated two-stage compiler before it can be published. It is kept
  // independent from the optional source-directory watcher so drag/drop,
  // scheduled imports, and API-created drafts all use the same review gate.
  useEffect(() => {
    if (!strictWorkerProject) {
      stopStrictIngestWorker()
      return
    }

    startStrictIngestWorker(strictWorkerProject)
    return () => {
      stopStrictIngestWorker()
    }
  }, [strictWorkerProject?.id, strictWorkerProject?.path])
  ...
  async function handleProjectOpened(proj: WikiProject) {
    ...
    // Start only after the awaited project shell/config hydration above. The
    // worker remains independent of source-watch enablement and handles every
    // draft created by UI, scheduled import, clipper, or API entry points.
    if (isCurrentProject(proj)) setStrictWorkerProject(proj)
  }
