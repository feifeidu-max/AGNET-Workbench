...
describe("scanAndImport failure handling", () => {
  ...
  beforeEach(() => {
    ...
    mocks.stageIngestSource.mockResolvedValue({ id: "draft-1" })
  ...
  it("does not mark changed files imported when staging fails", async () => {
    mocks.stageIngestSource.mockRejectedValue(new Error("queue stopped"))

    await scanAndImport(project, "/Users/me/inbox")

    expect(mocks.stageIngestSource).toHaveBeenCalled()
  ...
  it("continues scanning when one file is locked or unreadable", async () => {
    ...
    mocks.stageIngestSource.mockResolvedValue({ id: "draft-ok" })

    await scanAndImport(project, "/Users/me/inbox")

    expect(mocks.stageIngestSource).toHaveBeenCalledTimes(1)
    expect(mocks.stageIngestSource).toHaveBeenCalledWith(project.path, "/Users/me/inbox/ok.pdf")
    expect(mocks.writeFileAtomic).toHaveBeenCalled()
  })
  ...
  it("skips large files before hashing or copying", async () => {
    ...
    expect(mocks.stageIngestSource).not.toHaveBeenCalled()
  })
  ...
  it("does not copy unattended json/yaml/xml config files", async () => {
    ...
    mocks.stageIngestSource.mockResolvedValue({ id: "draft-note" })

    await scanAndImport(project, "/Users/me/inbox")

    expect(mocks.stageIngestSource).toHaveBeenCalledTimes(1)
    expect(mocks.stageIngestSource).toHaveBeenCalledWith(project.path, "/Users/me/inbox/notes.md")
  })
})
