...
import { stageIngestSource } from "@/commands/ingest-gate"
...
export async function importSourceFiles(
  ...
  for (const sourcePath of sourcePaths) {
    ...
    try {
      const draft = await stageIngestSource(pp, sourcePath)
      importedPaths.push(`${pp}/.llm-wiki/staging/${draft.id}`)
    } catch (err) {
      console.error(`Failed to stage ${originalName} for review:`, err)
    }
  }
...
export async function importSourceFolder(
  ...
  const naturallyOrderedFiles = [...allowedFiles].sort((a, b) =>
    naturalCompare(getRelativePath(a, sourceRoot), getRelativePath(b, sourceRoot)),
  )

  const stagedPaths: string[] = []
  for (const sourcePath of naturallyOrderedFiles) {
    try {
      const draft = await stageIngestSource(pp, sourcePath)
      stagedPaths.push(`${pp}/.llm-wiki/staging/${draft.id}`)
    } catch (err) {
      console.error(`Failed to stage ${sourcePath} for review:`, err)
    }
  }
