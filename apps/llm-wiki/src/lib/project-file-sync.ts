...
import {
  ...
  setIngestEmbeddingStatus,
  stageIngestSource,
  type StrictIngestDraft,
...
let unlistenPublished: UnlistenFn | null = null
let startSeq = 0
...
export async function startProjectFileSync(
  ...
  unlistenPublished = await listen<StrictIngestPublication>("strict-ingest://published", (event) => {
    const current = useWikiStore.getState().project
    if (!current || event.payload.projectId !== current.id) return
    void indexStrictPublication(current, {
      id: event.payload.draftId,
      publishedPages: event.payload.pagePaths,
    })
  })
  ...
  try {
    ...
    const pendingPublications = await pendingIngestPublications(project.path).catch(() => [])
    for (const draft of pendingPublications) {
      await indexStrictPublication(project, draft)
    }
  } catch (err) {
    unlistenQueue?.()
    unlistenChanged?.()
...
async function enqueueRawSourceChanges(project: WikiProject, tasks: FileChangeTask[]): Promise<void> {
  ...
  for (const relativePath of paths) {
    const absolutePath = `${normalizePath(project.path)}/${relativePath}`
    try {
      await stageIngestSource(project.path, absolutePath)
      await deleteFile(absolutePath)
    } catch (err) {
      console.error("[file-sync] failed to stage raw source for review:", err)
      useFileSyncStore.getState().setLastError(
        `Failed to stage ${relativePath}: ${err instanceof Error ? err.message : String(err)}`,
      )
    }
  }
}

interface StrictIngestPublication {
  projectId: string
  projectPath: string
  draftId: string
  sourcePath?: string
...
async function indexStrictPublication(
  project: WikiProject,
  draft: Pick<StrictIngestDraft, "id" | "publishedPages">,
): Promise<void> {
  const config = useWikiStore.getState().embeddingConfig
  if (!config.enabled || !config.model) {
    await setIngestEmbeddingStatus(project.path, draft.id, "disabled").catch(() => {})
    await refreshAfterFileChanges(project, draft.publishedPages)
    return
  }

  await setIngestEmbeddingStatus(project.path, draft.id, "indexing").catch(() => {})
  try {
    const { embedPage } = await import("@/lib/embedding")
    for (const relativePath of draft.publishedPages) {
      const absolutePath = `${normalizePath(project.path)}/${relativePath}`
      const content = await readFile(absolutePath)
      const pageId = getFileStem(relativePath)
      const title = content
        .split(/\r?\n/)
        .find((line) => /^#\s+/.test(line))
        ?.replace(/^#\s+/, "")
        .trim() || pageId
      await embedPage(project.path, pageId, title, content, config)
    }
    await setIngestEmbeddingStatus(project.path, draft.id, "indexed")
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err)
    await setIngestEmbeddingStatus(project.path, draft.id, "failed", message).catch(() => {})
    console.error("[strict-ingest] failed to index approved draft:", err)
  }
  await refreshAfterFileChanges(project, draft.publishedPages)
}
