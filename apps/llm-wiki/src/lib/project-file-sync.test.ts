...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    enqueueBatch: vi.fn(async (_projectId: string, _files: unknown[]) => []),
    stageIngestSource: vi.fn(async (_projectPath: string, _sourcePath: string) => ({ id: "draft" })),
    pendingIngestPublications: vi.fn(async () => []),
    setIngestEmbeddingStatus: vi.fn(async () => ({})),
...
vi.mock("@/commands/ingest-gate", () => ({
  ...
  pendingIngestPublications: mocks.pendingIngestPublications,
  setIngestEmbeddingStatus: mocks.setIngestEmbeddingStatus,
}))
...
describe("project file sync", () => {
  beforeEach(async () => {
    ...
    mocks.stageIngestSource.mockImplementation(async (projectPath: string, sourcePath: string) => {
      const relativePath = sourcePath.replace(/^\/tmp\/a\//, "")
      await mocks.enqueueBatch("A", [{ sourcePath: relativePath, folderContext: "" }])
      return { id: `draft-${projectPath}-${sourcePath}` }
    })
