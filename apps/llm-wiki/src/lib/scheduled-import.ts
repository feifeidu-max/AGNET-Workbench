...
import { isIngestableSourcePath } from "@/lib/source-lifecycle"
...
export async function scanAndImport(
  ...
  try {
    ...
    const changedFiles: Array<{ key: string; md5: string; draftId: string }> = []
    ...
    for (const file of collectFiles(tree)) {
      try {
        ...
        const draft = await stageIngestSource(projectPath, sourcePath)
        changedFiles.push({ key, md5, draftId: draft.id })
      } catch (err) {
