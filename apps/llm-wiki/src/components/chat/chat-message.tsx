...
import { stageGeneratedWikiPage } from "@/commands/ingest-gate"
...
function SaveToWikiButton({ content, visible }: { content: string; visible: boolean }) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const targetPath = `wiki/queries/${fileName}`
      ...
      await stageGeneratedWikiPage({
        projectPath: pp,
        title,
        targetPath,
        content: frontmatter + cleanContent,
        origin: "chat-save-to-wiki",
      })
      setSaved(true)
      setTimeout(() => setSaved(false), 2000)
      return

      // Legacy direct-write path retained below only as historical context;
      // strict mode returns after staging the proposal above.

      // Update index.md — append under ## Queries section
      /* index/log updates happen only when the staged draft is approved. */
