...
pub struct AgentChatRequest {
    pub message: String,
    // Read-only turns may retrieve and synthesize knowledge, but must not
    // expose or execute any write/process tools. Accept the snake_case alias
    // as well because a few non-JavaScript callers serialize Rust field names.
...
mod tests {
    ...
    #[test]
    fn chat_request_accepts_read_only_camelcase_and_snakecase_aliases() {
        let camel: AgentChatRequest = serde_json::from_value(serde_json::json!({
            "message": "read",
            "readOnly": true
        }))
        .unwrap();
        let snake: AgentChatRequest = serde_json::from_value(serde_json::json!({
            "message": "read",
            "read_only": true
        }))
        .unwrap();

        assert!(camel.read_only);
        assert!(snake.read_only);
    }
