...
impl AgentRuntime {
    ...
    pub async fn run_once_with_cancel_and_events(
        ...
        let shell_call = if request.read_only || skills.is_empty() {
            None
            ...
            .find(|call| call.tool.trim() == "wiki.write_page")
        {
            ...
            let tool_context = self.tool_context_for_request(&request);
            ...
            .and_then(|value| {
                ...
                Ok(output) => {
                    let staged_draft = output.draft.clone();
                    let operation = output.operation.clone();
                    if staged_draft.is_none() {
                        emit_event(
                            &mut events,
                            &event_sink,
                    ...
                    tool_emit_event(
                        ...
                        AgentToolEvent {
                            tool: "wiki.write_page".to_string(),
                            status: "completed".to_string(),
                            detail: Some(if let Some(draft) = staged_draft.as_ref() {
                                format!("staged draft {} ({:?})", draft.id, draft.status)
                            } else {
                                path.to_string()
                            }),
                        },
                    ...
                    emit_event(
                        ...
                        AgentEvent::tool_end(
                            "wiki.write_page",
                            Some(if let Some(draft) = staged_draft.as_ref() {
                                format!("staged draft {} awaiting review", draft.id)
                            } else {
                                path.to_string()
                            }),
                        ),
                    );
                    retrieval_parts.push(if let Some(draft) = staged_draft.as_ref() {
                        format!(
                            "wiki.write_page staged draft {} for {} (operation={}); it is not published until review approval.",
                            draft.id,
                            path,
                            operation.as_deref().unwrap_or("create")
                        )
                    } else {
                        format!("wiki.write_page wrote {path}.")
                    });
                }
                Err(err) => {
                    tool_emit_event(
                ...
                if !is_shell_command_approved(command, &request.approved_shell_commands) {
                    ...
                    match execute_tool_with_cancellation(
                        tool_registry.execute(
                            ...
                            self.tool_context_for_request(&request),
                        ),
                        cancellation.as_ref(),
                    )
                    .await
...
impl AgentRuntime {
    ...
    async fn run_agent_loop(
        ...
        for iteration in 0..max_iterations {
            ...
            let (system, user) = if must_finalize {
                ...
                (
                    build_agent_loop_system_for_request(
                        &built_context.system,
    ...
    async fn execute_agent_loop_tool(
        ...
        let result = execute_tool_with_cancellation(
            tool_registry.execute(tool, input, self.tool_context_for_request(request)),
            cancellation,
        )
        .await;
    ...
    fn record_loop_tool_success(
        ...
        match tool {
            ...
            "wiki.write_page" => {
                ...
                Ok(if let Some(draft) = staged_draft.as_ref() {
                    format!(
                        "staged draft {} for {path} (operation={}); not published until review approval",
                        draft.id,
                        operation.as_deref().unwrap_or("create")
                    )
                } else {
    ...
    fn tool_context_for_request(&self, request: &AgentChatRequest) -> tools::ToolContext<'_> {
        tools::ToolContext {
            ...
            // Agent-generated Wiki writes are always review-first. The
            // low-level tools API remains available to explicit legacy
            // callers with `strict_ingest: false`.
            strict_ingest: true,
        }
    }
    ...
    async fn plan_tools_with_model(
        ...
        check_cancel(cancellation)?;
        let mut available = vec!["wiki.search", "source.search", "graph.search"];
        if !read_only {
            available.push("wiki.write_page");
        }
        if tools.web {
        ...
        let write_policy = if read_only {
            "This is a read-only turn. Plan only tools listed under Available tools; mutation and process actions are unavailable and will be rejected."
        } else {
            "Use wiki.write_page only when the user explicitly asks to create or update a wiki page; generated pages enter the review queue before publication. Use workspace.write_file for generated artifacts under agent-workspace and shell.exec only when an active skill requires a command-line operation."
        };
        let user = format!(
            "User request:\n{message}\n\nSkill context:\n{skill_context}\n\nAvailable tools: {}\n\nAgent workspace for generated files: {workspace}\n\nReturn JSON exactly like {{\"toolCalls\":[{{\"tool\":\"wiki.search\",\"query\":\"short query\"}}]}}. Use an empty array when no tool is needed. The skill context and tool list above are already available to the assistant; do not call any tool merely to list, explain, or summarize the currently available skills, tools, modes, or Agent capabilities. Use wiki.search for factual or topical retrieval. Prefer graph.search for relationships, dependencies, neighborhoods, backlinks, or connections between entities; pass concise entity or concept names instead of the full question. The planner may select both when the answer needs page content and graph structure. Prefer web.search only for current/external information. Prefer anytxt.search only for user files outside the wiki. Use skill.read_file for Markdown/reference files inside an active skill directory. {write_policy}",
            available.join(", "),
        );
        let client = LlmClient::new(config.clone())?
            .structured_task_config(agent_structured_max_tokens(!skills.is_empty()));
...
fn build_agent_loop_system(base_system: &str, retrieval_mode: AgentRetrievalMode) -> String {
    build_agent_loop_system_for_request(base_system, retrieval_mode, false)
}

fn build_agent_loop_system_for_request(
    base_system: &str,
    retrieval_mode: AgentRetrievalMode,
    read_only: bool,
) -> String {
...
Use wiki.write_page only when the user explicitly asks to create or update a wiki page. Existing pages are create-only unless allowOverwrite is explicitly justified by the user's request.{smart_retrieval}"
    ...
    if read_only {
        // Keep write/process tool names out of the model-visible protocol in
        // addition to enforcing the backend permission check.
        prompt = prompt
            .lines()
            .filter(|line| {
                !line.contains("wiki.write_page")
                    && !line.contains("workspace.write_file")
                    && !line.contains("workspace.append_file")
                    && !line.contains("shell.exec")
            })
            .collect::<Vec<_>>()
            .join("\n");
        prompt.push_str(
            "\nRead-only turn: retrieval and final answers only; Wiki, workspace, and shell writes/processes are unavailable.\n",
        );
    }
    prompt
}
...
fn build_agent_loop_user(
    ...
    if request.tools.wiki {
        ...
        if !request.read_only {
            out.push_str(
                "- wiki.write_page: create a wiki markdown page when explicitly requested; it enters the review queue before publication.\n",
            );
        }
    }
    if request.tools.web {
    ...
    if request.read_only {
        out.push_str(
            "- Read-only turn: Wiki/workspace writes and shell/process tools are unavailable.\n",
        );
    }
    if observations.is_empty() {
...
fn require_tool_permission(
    ...
    if request.read_only
        && matches!(
            tool,
            "wiki.write_page" | "workspace.write_file" | "workspace.append_file" | "shell.exec"
        )
    {
        return Err(format!("{tool} is disabled for read-only turns"));
    }
    match tool {
        "wiki.search" => {
            if !request.tools.wiki {
...
mod tests {
    ...
    #[test]
    fn read_only_turn_rejects_all_mutating_and_process_tools() {
        let request = AgentChatRequest {
            read_only: true,
            ..AgentChatRequest::default()
        };
        ...
        for tool in [
            ...
            let error = require_tool_permission(tool, &request, &policy).unwrap_err();
            assert!(error.contains("read-only"), "{tool}: {error}");
        }
    }

    #[test]
    fn read_only_loop_prompts_do_not_expose_write_or_process_tools() {
        let request = AgentChatRequest {
            read_only: true,
            skills: vec!["demo".to_string()],
            ..AgentChatRequest::default()
        ...
        let system =
            build_agent_loop_system_for_request("base", AgentRetrievalMode::Standard, true);
        let user = build_agent_loop_user("base", &request, &[], &[], 0, 4, false);
        for prompt in [system, user] {
            assert!(!prompt.contains("wiki.write_page"));
            assert!(!prompt.contains("workspace.write_file"));
            assert!(!prompt.contains("workspace.append_file"));
