...
pub struct ToolContext<'a> {
    ...
    /// Deny all mutating/process tools for a read-only Agent turn. This is a
    /// backend guard in addition to prompt/tool-list filtering.
    pub read_only: bool,
    /// Runtime Agent writes always go through the strict review queue. Keep
    /// the switch on the context so legacy direct callers can retain the
    /// historical low-level write helper when they explicitly need it.
...
impl ToolRegistry for BuiltinToolRegistry {
    ...
    fn execute<'a>(
        ...
        Box::pin(async move {
            ...
            {
                return Err(format!("{name} is disabled for read-only turns"));
            }
            match name {
                "wiki.write_page" => {
                    let path = input
                    ...
                    let allow_overwrite = input
                        ...
                        .unwrap_or(false);
                    let output = if context.strict_ingest {
                        stage_wiki_page_draft(context.project_path, path, content)?
                    } else {
...
pub struct WikiWriteOutput {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub previous_content: Option<String>,
    /// Present when the runtime strict-ingest path staged a proposal instead
    /// of changing the published Wiki tree.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub draft: Option<crate::ingest_gate::IngestDraft>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Stage a generated Wiki page behind the strict review gate. The published
/// `wiki/` tree is deliberately untouched until a reviewer approves the
/// resulting draft.
fn stage_wiki_page_draft(
    project_path: &str,
    rel_path: &str,
    content: &str,
) -> Result<WikiWriteOutput, String> {
    ...
    let rel = normalize_wiki_write_path(rel_path)?;
    let target = safe_project_join(project_path, &rel)?;
    let existed_before = target.is_file();
    let operation = if target.exists() { "update" } else { "create" };
    ...
    let draft = crate::ingest_gate::create_generated_page_draft(
        project_path,
        &title,
        &rel,
        content,
    ...
    Ok(WikiWriteOutput {
        ...
        existed_before,
        // A staged proposal has not changed a published file, so there is no
        // rollback snapshot to expose to the desktop event stream.
...
mod tests {
    ...
    #[tokio::test]
    async fn read_only_registry_rejects_writes_and_processes_without_side_effects() {
        let root = std::env::temp_dir().join(format!("llm-wiki-read-only-{}", Uuid::new_v4()));
        ...
        let calls = [
            (
                "wiki.write_page",
                json!({"path": "wiki/new.md", "content": "# New"}),
            ),
            (
                "workspace.write_file",
                json!({"path": "artifact.txt", "content": "x"}),
            ),
            (
                "workspace.append_file",
                json!({"path": "artifact.txt", "content": "x"}),
            ),
            ("shell.exec", json!({"command": "echo should-not-run"})),
        ];
        for (name, input) in calls {
            let error = registry
                .execute(name, input, context.clone())
                .await
                .unwrap_err();
            assert!(error.contains("read-only"), "{name}: {error}");
        }
        assert!(!root.join("wiki/new.md").exists());
        assert!(!root.join(AGENT_WORKSPACE_DIR).join("artifact.txt").exists());
        let _ = fs::remove_dir_all(root);
    }

    #[tokio::test]
    async fn strict_wiki_write_stages_create_and_update_proposals() {
        let root = std::env::temp_dir().join(format!("llm-wiki-strict-write-{}", Uuid::new_v4()));
        fs::create_dir_all(root.join("wiki")).unwrap();
        fs::write(root.join("wiki/existing.md"), "# Before").unwrap();
        let registry = BuiltinToolRegistry;
        let context = ToolContext {
            project_path: root.to_str().unwrap(),
            embedding_config: None,
        ...
        let created = registry
            ...
            .await
            .unwrap();
        let created_id = created["draft"]["id"].as_str().unwrap().to_string();
        assert_eq!(created["operation"], "create");
        assert_eq!(created["kind"], "wiki-draft");
        assert!(!root.join("wiki/new.md").exists());
        let created_detail =
            crate::ingest_gate::draft_detail(root.to_str().unwrap(), &created_id).unwrap();
        assert_eq!(
            created_detail["proposal"]["changes"][0]["operation"],
            "create"
        );
        ...
        let updated = registry
            .execute(
                "wiki.write_page",
                json!({"path": "wiki/existing.md", "content": "# After"}),
                context,
            )
            .await
            .unwrap();
        let updated_id = updated["draft"]["id"].as_str().unwrap().to_string();
        assert_eq!(updated["operation"], "update");
        assert_eq!(
            fs::read_to_string(root.join("wiki/existing.md")).unwrap(),
            "# Before"
        );
        let updated_detail =
            crate::ingest_gate::draft_detail(root.to_str().unwrap(), &updated_id).unwrap();
        assert_eq!(
            updated_detail["proposal"]["changes"][0]["operation"],
            "update"
        );
        let _ = fs::remove_dir_all(root);
    }
