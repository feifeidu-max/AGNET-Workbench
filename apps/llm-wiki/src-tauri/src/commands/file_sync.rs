...
// Strict-ingest publication can atomically move a large PDF on a slow disk;
// keep watcher suppression longer than the debounce window so it cannot be
// mistaken for a new untrusted source while the publication event is queued.
const APP_WRITE_IGNORE_MS: i64 = 60_000;
...
/// Record a strict-ingest publication in the watcher snapshot without
/// emitting legacy source-ingest tasks. The strict gate emits its own
/// publication event for embedding and UI refresh.
pub fn sync_app_written_paths(project_path: &str, relative_paths: &[String]) -> Result<(), String> {
    let root = PathBuf::from(project_path);
    let rels = relative_paths
        .iter()
        .filter_map(|value| normalize_rel_path(Path::new(value)))
        .collect::<BTreeSet<_>>();
    for rel in &rels {
        mark_app_write_path(&root.join(rel));
    }
    sync_snapshot_paths(&root, rels)
}
