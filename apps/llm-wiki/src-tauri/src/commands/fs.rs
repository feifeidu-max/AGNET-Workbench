...
/// Read the PDF document-info fields used by the strict ingest gate.
/// Text extraction and metadata extraction deliberately use separate Pdfium
/// sessions so the existing markdown extractor remains unchanged.
pub(crate) fn read_pdf_metadata_fields(
    path: &str,
    ...
    let document = pdfium
        ...
        .map_err(|error| format!("Failed to read PDF metadata for '{path}': {error}"))?;
    ...
    Ok((
        value(PdfDocumentMetadataTagType::Title),
        value(PdfDocumentMetadataTagType::Author),
        value(PdfDocumentMetadataTagType::CreationDate),
    ))
}
...
mod tests {
    ...
    fn allow_absolute_write_paths() {
        let native_absolute = std::env::temp_dir().join("project/wiki/sources/page.md");
        assert!(native_absolute.is_absolute());
        assert!(require_absolute_path("write_file", &native_absolute.to_string_lossy()).is_ok());
