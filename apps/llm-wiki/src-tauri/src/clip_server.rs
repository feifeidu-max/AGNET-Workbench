...
static PENDING_CLIPS: Mutex<Vec<(String, String, String)>> = Mutex::new(Vec::new()); // (projectPath, draftId, targetPath)
...
fn handle_clip(body: &str) -> String {
    ...
    let relative_path = format!("wiki/clips/{}-{}.md", base_name, &unique[..8]);
    let draft = match crate::ingest_gate::create_generated_page_draft(
        &project_path,
        title,
        &relative_path,
        &markdown,
        ...
        Ok(draft) => draft,
        Err(error) => {
            return serde_json::json!({ "ok": false, "error": error.message }).to_string()
        }
    };

    // Notify the frontend for activity refresh only. The clip is already a
    // strict draft and must never be sent through legacy source ingest.
    if let Ok(mut pending) = PENDING_CLIPS.lock() {
        pending.push((project_path, draft.id.clone(), relative_path.clone()));
    }
