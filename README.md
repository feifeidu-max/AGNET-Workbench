...
| 微信网关 | 扫码绑定个人微信（iLink），多成员每人独立 bot/会话，共享知识库；支持微信内 `帮我导入` 与 `批准/拒绝` | 已完成；`DM_POLICY=open` 下任意好友可直接私聊，无上限 |
...
4. 微信内直接发 `https://mp.weixin.qq.com/s/... 帮我导入` 即可抓取并创建草稿；随后在微信中回复 `批准` 或 `拒绝` 即可完成审核，无需打开 Studio。
