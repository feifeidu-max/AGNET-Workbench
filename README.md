...
1. 从 Studio 左侧打开“LLM Wiki”。这是唯一的知识库前端；顶部可切换现有项目，或在“概览”中新建受控项目。
2. 在“审核”中选择一篇或多篇原生 PDF。状态会依次经过解析、草稿和待审核；重复 PDF 由 SHA-256 识别。
3. 在草稿详情中查看拟新增/修改页面、当前内容、拟发布内容和证据定位，再选择“批准”、“退回”或“拒绝”。只有批准后的论文才进入可信库。
4. 在“Wiki”中浏览页面树、编辑或预览 Markdown、查看正向/反向链接、创建缺失页面，并从“历史”恢复文件版本。
5. 在“来源库”打开可信 PDF 或相关 Wiki 页面；在“检索”和“知识图谱”中只搜索与显示已发布内容。
6. 在“对话”中进行只读知识库问答。它不会写入 Wiki 或执行终端命令；“深度研究”可以显式启用外部搜索，研究结果只能“送审为 Wiki 草稿”。外部论文题录进入“待读候选箱”，不会自动下载或入库。
7. “检查”用于发现缺失 frontmatter 和未解析链接；“Skills”选择只读对话可使用的项目/用户 Skills；“设置”显示受控服务、Web Clipper 和维护状态。
