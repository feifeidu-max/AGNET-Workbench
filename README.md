...
AGNET 是面向个人研究与日常工作的 Windows 本地智能工作台。它不重新实现 Agent 核心，而是在固定版本的 Hermes Studio、Hermes Agent 和 LLM Wiki 上增加了论文知识库、微信多成员网关与记忆管理能力。

本仓库交付的是 **Windows 单用户、仅限 `localhost`、非商用验证原型**。完成本 README 的首次安装后，使用者可以在本机打开 `http://127.0.0.1:8648`，上传论文、审核知识、使用 Hermes 对话、管理记忆，并通过微信与知识库直接交互。
...
> 不需要安装 Ollama，也不需要执行 `ollama pull bge-m3`。当前知识库固定使用 **关键词检索 + 知识图谱扩展 + 可选 LLM 重排**，不生成 embedding、不运行向量数据库或本地生成模型。**知识库支持 LLM 语义富化**（离线摘要/主题分类/实体关系抽取/富化标签）与 **微信多成员独立对话**（每人扫码绑定专属网关，共享同一知识库）。
...
| 入口 | 用法 | 当前交付范围 |
| --- | --- | --- |
| 个人工作台 | 查看今日论文、待审核数、可信知识库规模、模型与网关健康 | 已完成；新增微信多成员在线/对话中实时看板 |
| Hermes 对话 | 创建会话、选择 Profile 和记忆模式，使用已配置的模型进行对话 | 已完成；默认 `mimo-v2.5 / opencode-go`，密钥已预置，支持 `xhigh` 推理与 200k 上下文 |
| LLM Wiki | 在 Studio 内完成项目创建/切换、论文审核、Wiki 树与编辑、文件历史、页面链接、来源库、检索、知识图谱、审核、对话、深度研究、Skills、检查和设置 | 已完成；检索为关键词 + 图谱 + 可选 LLM 重排，知识图谱支持 LLM 语义增强（语义摘要/主题分类/实体关系抽取/富化标签与关系，覆盖层可回退，支持停止） |
| 微信网关 | 扫码绑定个人微信（iLink），多成员每人独立 bot/会话，共享知识库；支持微信内 `帮我导入` 与 `批准/拒绝` | 已完成；`DM_POLICY=open` 下任意好友可直接私聊，无上限；单链导入自动通过审核直接入库（自动发现/订阅文章仍走人工审核），语义富化可在知识页手动触发 |
...
1. 打开 `http://127.0.0.1:8648/#/hermes/personal-workbench`，在 **微信网关** 卡片点 **扫码登录**，用主账号微信扫码完成首次绑定。
2. 在同一卡片的 **多成员接入** 区点 **添加成员（扫码）**，让同事用**他自己的微信**扫弹出的二维码并确认，填备注后即生成独立会话（卡片显示 `在线`）。
3. 成员无需加好友，直接在微信中与自己的专属 bot 私聊即可；消息走独立会话但共享同一知识库。卡片实时显示 `在线/对话中` 与最近活动。
4. 微信内直接发 `https://mp.weixin.qq.com/s/... 帮我导入` 即可抓取并**自动通过审核直接入库**（用户发链接导入即视为审核决定，无需再回复批准；若个别情况自动入库降级，Hermes 会提示回复 `批准` 重试）。知识页的 **AI 增强** 为手动可选步骤，可为文章补齐语义摘要/实体/关系并丰富知识图谱；如需让导入在入库前自动完成增强，可设 `LLM_WIKI_INGEST_ENRICHMENT=1`。

#### 拓展：AI 语义增强与重排

1. 打开 `http://127.0.0.1:8648/#/hermes/knowledge?tab=graph`，顶部 **AI 语义增强** 面板点 **AI 增强**（离线富化，`mimo-v2.5`），进度条与 `已完成/失败` 实时更新；中途可点 **停止增强**。
2. 检索时勾选 **LLM 重排**，在关键词+图谱混合检索后追加 LLM 相关性重排（失败自动回退）。
