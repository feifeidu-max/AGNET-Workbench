...
| 微信网关 | 扫码绑定个人微信（iLink），多成员每人独立 bot/会话，共享知识库；支持微信内 `帮我导入` 与 `批准/拒绝` | 已完成；`DM_POLICY=open` 下任意好友可直接私聊，无上限；导入文章在批准入库前自动走完离线语义增强（摘要/主题/实体/关系），确保进入知识图谱 |
...
4. 微信内直接发 `https://mp.weixin.qq.com/s/... 帮我导入` 即可抓取并创建草稿；后台会先执行**发布前离线语义增强**（语义摘要/主题分类/实体抽取 + 与库内文章的关系标注，约 1-2 分钟），完成后在微信中回复 `批准` 或 `拒绝` 即可完成审核，无需打开 Studio。批准时若提示“草稿正在离线语义处理”，约 1 分钟后重新发送 `批准` 即可；增强失败的草稿会标记失败，不会被静默入库。
