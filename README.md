...
| LLM Wiki 管理 | 上传 PDF、查看审核队列、批准或退回草稿、搜索可信知识、问答、查看待读候选、知识图谱和本机服务状态 | 已完成；检索为关键词 + 图谱，不含向量检索 |
...
启动器会注册配置中的项目；只有一个存在的路径时会自动选为当前项目。存在多个项目时，从 Studio 左侧的“LLM Wiki”一级入口进入“LLM Wiki 管理”页切换，不需要打开独立 LLM Wiki。
...
重新打开 PowerShell 后，启动器会把这些变量交给后台服务。密钥不写入仓库、本地配置、Wiki 或备份；Studio 的“LLM Wiki 管理”会显示问答模型是否可用。
...
1. 从 Studio 左侧打开“LLM Wiki”。默认会显示“LLM Wiki 管理”；切换到“论文归纳与审核”后，在上传区选择一篇或多篇原生 PDF。
...
创建项目目录并让其路径与 `WikiProjectPaths` 逐字匹配。建议首次交付时只保留一个存在的项目路径，启动器即可自动选择它；需要切换多个项目时，从 Studio 左侧的“LLM Wiki”一级入口进入“LLM Wiki 管理”操作；任何需要备份的项目都必须列在该数组中。
